...
!  use FoX_wxml, only: xmlf_t, xmlf_NewId, xmlf_OpenTag
  ...
  interface kmlCreateCells
    ...
    module procedure kmlCreateCells3  !this is double precision, need to change name to kmlCreateCells3_dp later
...
! tmp correct by GT
    long_inc = (east-west)/m ! Increment in latitude
    lat_inc = (north-south)/n ! Increment in longitude
...
! change by GT to plot from north to south in order to match data structure
...
!!test by using GT code, this version works, 07032009!!!!!!!!!!!!!!!!
           do ic=1, size(contour_values)-1
            if (values(i,j) .lt. contour_values(1)) then
              ...
              k = ic+1
           ! else if (average .GT. contour_values(ic+1)) then
           !    k = ic+2
            end if
           end do
        !print*, 'k in gt version =', k
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!! toby's version, replace this by previous block in order to fix the assigning color problem
!            k = 0
!          else
...
! chagen by GT 30012008
    ...
    do i = 1, m
      ...
      !print*,'long=',long
      do j = 1, n         
        !lat = south+lat_inc*(j-0.5) ! change i-0.5 to j-0.5 GT 09032009, fix bug
        ! change by GT 09032009, plot cells from north to south, to mathc data structure
        lat = north-lat_inc*(j-0.5) ! change i-0.5 to j-0.5 GT 09032009, fix bug
        !print*,'lat=',lat
        if (present(mask)) then
          if (values(i,j)>=mask) cycle
        endif
...
!        square(2, :) = (/lat, lat, lat+lat_inc, lat+lat_inc/)       ! y-coords
        ...
        if (present(contour_values)) then
        !print*,'run if contour_values exist'
!!test by using GT code, this version works, 07032009!!!!!!!!!!!!!!!!   
         do ic=1, size(contour_values)-1
           if (values(i,j) .lt. contour_values(1)) then
             k = ic
...
!!$    end do
    ...
    do i = 1, m-1
      do j = 1, n-1
        ...
        if (present(contour_values)) then
!!test byc=1, size(contour_values)-1
        do ic=1, size(contour_values)-1
          if (average .lt. contour_values(1)) then
             k = ic
          else if ((average >= contour_values(ic)) .and. (average < contour_values(ic+1))) then
...
!          if (average<contour_values(1)) then
!            k = 0
!            print*,'when ave<CV k=',k          
!          else
!            print*,'size of contour_values', size(contour_values)
!            do ic = 1, size(contour_values)
!              ! change contour_values(i) to contour_values(ic) by GT, it seems a bug
!               if (average>contour_values(ic)) then
!                ! change k=i to k=ic by GT, seems a bug 
!                k = ic
!                print*,'ave > CV k=',k                        
!                exit
!              endif
